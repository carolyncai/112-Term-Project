...
from BHMode import BHMode, BHMode2, BHMode3
...
# framework from pygamegame.py, from CA Lukas Peraza, from optional Pygame lecture
    def init(self):
    ...
    def keyPressed(self, keyCode, modifier):
        if self.mode == "titleScreen":
            ...
            if self.intro.inIntro == False:
                ...
                self.music_6pm.play(-1, fade_ms=500)
                self.mode = "RPG"
        elif self.mode == "RPG":
            ...
            if self.RPG.inBH:
                ...
                self.RPG.BH.music.play(-1, fade_ms=500)
                self.mode = "BH"
        elif self.mode == "BH":
            ...
            if self.realEnding.inRealEnding == False:
                ...
                self.music_credits.play(-1, fade_ms=500)
                self.mode = "credits"
    ...
    def timerFired(self, dt):
        if self.mode == "RPG":
            ...
            if self.RPG.completed:
                ...
                self.mode = "ending"
                self.music_6pm.fadeout(100)
                self.music_shop.play(0, fade_ms=300) 
                # i'll just let this fade out into silence
            ...
            if self.RPG.BH.completed:
                ...
                self.fading = True
                self.RPG.BH.music.stop()
                self.music_6pm.play(-1, fade_ms=300)
                self.mode = "RPG"
        elif self.mode == "shop":
